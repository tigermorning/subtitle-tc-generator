...
# **1차는 오역만 본다.** 전에는 이 프롬프트가 문체·간결성·문화 조정까지 한꺼번에
# 시켰다. 작업자 자료 569~579행이 나눠 놓은 것과 어긋났고, 한 번에 셋을 시키면 모델이
# 뒤섞어 어중간하게 낸다 — 그러면 가장 비싼 것(오역)에 쓸 여유가 없어진다.
#
#     1차   오역 없이. 투박한 한국어는 괜찮다              <- 여기
#     2차   맥락·용어·인물 관계·말투                       (`revise.SECOND_PASS`)
#     3차   말맛·간결·문장부호·글자 수                     (`revise.THIRD_PASS`)
#
...
def _base_rules(lang_label: str, formality_note: str = "") -> str:
    """ko/en과 같은 골격 — 오역 금지·구조 보존은 언어를 안 가리는 보편 원칙이다.
    바뀌는 건 목표 언어 이름과, 그 언어에 실제로 있는 문법 범주(격식 구분)뿐이다
    (2026-08-30, 영화A·영화B 정답지 대조를 위해 12개 언어 추가 — 어느
    언어가 tu/vous류 격식 구분을 문법으로 갖는지는 발주처 자료가 아니라 일반
    언어학적 사실이라 규칙 9의 "발주처 규정 추측 금지"와는 층이 다르다)."""
    lines = [
        f"당신은 영상 번역가입니다. 원어 대사를 {lang_label} 자막으로 **1차 번역**합니다.",
        "**볼 것은 하나입니다 — 뜻이 틀리지 않는 것.**",
        "",
        f"- 투박하거나 직역투인 {lang_label}는 괜찮습니다. 다듬는 것은 2차·3차에서 합니다.",
        "- **오역은 괜찮지 않습니다.** 뜻을 모르면 지어내지 말고 원문을 그대로 두세요.",
        "- 부정(not·never·no에 해당하는 원어 표현), 숫자, 사람·장소 이름을 빠뜨리지 마세요. 뜻이 뒤집힙니다.",
        "- 원문의 단어에 매달리지 말고 **의미**를 옮깁니다.",
    ]
    if formality_note:
        lines.append(formality_note)
    lines += [
        "- 자막은 한 줄에 하나입니다. **번호를 합치거나 나누지 마세요.**",
        "- 대괄호 안의 화자명·효과음, 음표, 태그는 그대로 둡니다.",
        "- 설명을 덧붙이지 말고 번역만 냅니다.",
...
_TV_NOTE = "- 상대와의 관계를 모르면 **격식체(존댓말에 해당하는 쪽)로 통일**합니다. 관계에 맞추는 것은 2차에서 합니다."
# **일본어 자막은 정중체가 기본이 아니다**(2026-09-08 실측 정정). 정답 자막 4작품
# 21,888개에서 です/ます 종결은 0~17%뿐이고 나머지는 상시체·체언종결이다(발주처
# 넷·장르 넷, 표는 `revise.py`의 일본어 프롬프트 주석). 전에는 이 자리가 "관계를
# 모르면 정중체로 통일"이었는데, 그러면 1차가 만들어 낸 です/ます를 2·3차가 전부
# 되돌려야 한다 — 애초에 만들지 않는 쪽이 싸다.
_JA_NOTE = "- 문말은 **상시체(だ·である·체언종결)를 기본**으로 씁니다. 화자가 분명히 정중하게 말하는 자리에서만 です·ます를 씁니다. 세밀한 조정은 2차에서 합니다."
_ZH_NOTE = "- 您/你 같은 존대 구분이 필요한 자리는 관계를 모르면 존대 쪽을 씁니다. 세밀한 조정은 2차에서 합니다."

SYSTEM_BY_LANG.update({
    "fr": _base_rules("프랑스어", _TV_NOTE),       # tu/vous 구분 활발
    "de": _base_rules("독일어", _TV_NOTE),         # du/Sie 구분 활발
    "it": _base_rules("이탈리아어", _TV_NOTE),      # tu/Lei 구분 활발
    "pt": _base_rules("포르투갈어", _TV_NOTE),      # tu/você 구분 활발(유럽 포르투갈어 기준)
    "nl": _base_rules("네덜란드어", _TV_NOTE),      # je/u 구분 활발
    "pl": _base_rules("폴란드어", _TV_NOTE),        # ty/pan·pani 구분 활발
    "ja": _base_rules("일본어", _JA_NOTE),
    "zh": _base_rules("중국어", _ZH_NOTE),
    # 격식 구분이 사실상 소멸한 언어(du-reformen 등) — 별도 안내 없이 보편
    # 원칙만 적용한다.
    "da": _base_rules("덴마크어"),
    "nb": _base_rules("노르웨이어"),
    "sv": _base_rules("스웨덴어"),
    "fi": _base_rules("핀란드어"),
})

LANG_NAMES: dict[str, str] = {
    "ko": "한국어", "en": "영어", "fr": "프랑스어", "de": "독일어", "it": "이탈리아어",
    "pt": "포르투갈어", "nl": "네덜란드어", "pl": "폴란드어", "ja": "일본어",
    "zh": "중국어", "da": "덴마크어", "nb": "노르웨이어", "sv": "스웨덴어", "fi": "핀란드어",
}
