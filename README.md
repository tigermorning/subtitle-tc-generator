...
갈래마다 전문 분야가 다르고 하나하나가 시간을 많이 잡아먹기 때문이다. 겹쳐서 받는
경우가 있어도 **TC+SDH 정도**다.
...
`TC+SDH` 또는 `TC+번역`을 통째로 맡고, 사람이 하는 일은 **감수**만 남는 상태.
...
```bash
python3 tests/run_tests.py     # 1078건 (GUI 제외)
cmd.exe /c "..\korean-subtitle-corrector\.venv\Scripts\python.exe tests\run_tests.py"   # 1078건
```
