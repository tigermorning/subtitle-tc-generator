...
platform·lang·kind·genre가 적혀 있으니, 그걸 읽어서 플래그를 조립하면 사람이
다시 타이핑할 일이 없어져 이 종류의 실수 자체가 구조적으로 사라진다.

    python tools/gen_from_ledger.py 예능A 15 -o 출력.srt

끝나면 원장의 `pipeline.tc_generated`를 자동으로 갱신한다(날짜·플래그·출력 경로).
`tc_verified`·`text_polished`·`delivered`는 이 도구가 손대지 않는다 — 그건 각자
별도 단계라 사람 판단이 들어가야 한다(CLAUDE.md 규칙 17).
"""
...
def main() -> int:
    ...
    out = args.out or (ROOT / ".tmp" / "최종초안" /
                        f"{args.work}_{args.episode}회_{work['lang']}{work['kind']}_초안.srt")
    out.parent.mkdir(parents=True, exist_ok=True)

    cmd = [
        sys.executable, "-m", "checker", "--generate",
        "--video", str(video),
        "--whisper-lang", work["lang"],
        "-p", work["platform"], "-l", work["lang"], "-k", work["kind"],
        "-o", str(out),
    ]
    if work.get("genre"):
    ...
    pipeline = ep.setdefault("pipeline", {})
